...
var animal_scenes = {
	"go": {"color": "blue", "texture": "res://objects/electric_blue_diamond.png"}
}


func _ready():
	object_count = Global.go_only_object_count
	round_count = 1
...
func start_game():
	...
	# make list based of the repeats
	...
	spawn_plan.shuffle()
	spawn_plan = spawn_plan.slice(0, object_count)
...
func create_screen_flash(is_nogo : bool):
	...
	if is_nogo:
		screen_flash_instance.set_flash_color(Color(1, 0.2, 0.2, 1))  # red
	else:
...
func create_ring(click_position: Vector2, is_nogo: bool):
	...
	if is_nogo:
		ring_instance.ring_color = Color(1, 0.2, 0.2, 1)  # red
	else:
...
func show_next_object():
	...
	removal_timer.wait_time = Global.object_time_visible
...
func end_game():
	...
	var omission_errors = Global.omission_mistake
	var accuracy_percent = total_correct_choices * 100.0 / total_objects
	var omission_percent = omission_errors * 100.0 / total_objects
...
func write_info_txt_file(
	...
	file.store_line("Mode: Go-Only")
	file.store_line("Player Name: %s" % Global.player_name)
	file.store_line("Player ID: %s" % Global.player_id)
	file.store_line("")
	
		# --- SETTINGS ---
	file.store_line("== Game Settings ==")
	file.store_line("Visual Feedback Type: " + Global.feedback_mode)
	
	if Global.feedback_mode == "border_flash":
		file.store_line(" - Screen Border Flash Size: %s (relative to screen height/width)" % fraction_strings.get(Global.screen_flash_size, str(Global.screen_flash_size)))
		...
		file.store_line(" - Screen Border Flash Size: -")
		file.store_line(" - Expanding Ring Radius: -")
	
	file.store_line("Spawn Interval: %.1f sec" % Global.spawn_interval)
	file.store_line("Object Visible Time: %.1f sec" % Global.object_time_visible)
	file.store_line("Total Objects Presented: %d" % (object_count_param * round_count_param))
	file.store_line("")
	...
	file.store_line("In Go-Only mode, all images are Go-objects.")
	file.store_line("Not clicking a Go-object within the time limit is considered an omission error.")
